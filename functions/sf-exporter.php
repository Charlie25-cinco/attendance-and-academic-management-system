...
class Sf1Parser {
    ...
    private function parseHeader(array $data): array {
        return [
            'school_name' => $this->firstCell($data, 3, range(2, 9)),
            'school_id' => $this->firstCell($data, 3, range(11, 17)),
            'district' => $this->firstCell($data, 3, range(19, 23)),
            ...
            'school_year' => $this->firstCell($data, 5, range(12, 19)),
            'grade_level' => $this->firstCell($data, 5, range(21, 24)),
            'track_strand' => $this->firstCell($data, 5, range(26, 34)),
            'section' => $this->firstCell($data, 7, range(1, 10)),
            'course_tvl' => $this->firstCell($data, 7, range(12, 20)),
        ];
    }
    ...
    private function parseStudents(array $data): array {
        ...
        for ($row = 11; $row <= 100; $row++) {
            ...
            $lrn = $this->cell($data, $row, 1); $lastName = $this->cell($data, $row, 2); $firstName = $this->cell($data, $row, 3);
            if ($lrn === '' && $lastName === '' && $firstName === '') continue;
            $markerText = strtoupper($lrn . ' ' . $lastName . ' ' . $firstName);
            if (str_contains($markerText, '<===') || str_contains($markerText, 'REQUIRED INFORMATION') || str_contains($markerText, 'NAME OF SCHOOL, DATE')) {
                continue;
            }
            $student = [
                'lrn' => $lrn, 'last_name' => $lastName, 'first_name' => $firstName,
                'name_extension' => $this->cell($data, $row, 4), 'middle_name' => $this->cell($data, $row, 5),
...
class Sf1Exporter {
    ...
    private function fetchStudents(int $gradeLevel, string $section, ?string $track, string $academicYear): array {
        $sql = "SELECT u.id, u.lrn, u.first_name, u.middle_name, u.last_name, u.sex, u.date_of_birth, u.contact_number, u.address, COALESCE(u.religion, '') as religion, COALESCE(u.name_extension, '') as name_extension, COALESCE(u.father_name, '') as father_name, COALESCE(u.mother_name, '') as mother_name, COALESCE(u.guardian_name, '') as guardian_name, COALESCE(u.guardian_relationship, '') as relationship FROM users u WHERE u.role = 'student' AND u.status = 'active' AND u.grade_level = ? AND LOWER(TRIM(COALESCE(u.section, ''))) = LOWER(TRIM(?))";
