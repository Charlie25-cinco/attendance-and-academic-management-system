...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'get_messages') {
        ...
        $stmt = $db->prepare("
            ...
            LIMIT 50
        ");
        $stmt->execute([$teacherId, $parentId, $parentId, $teacherId, $lastId]);
        ...
        foreach ($msgs as $msg) {
            $result[] = [
                'id' => (int)$msg['id'],
                'is_mine' => (int)$msg['from_user_id'] === $teacherId,
...
if ($action === 'send_message') {
    ...
    if (mb_strlen($message) > 1000) {
        echo json_encode(['success' => false, 'message' => 'Message too long (max 1000 chars).']);
        exit();
    }
    ...
    $senderName = trim((string)($_SESSION['first_name'] ?? 'Teacher') . ' ' . (string)($_SESSION['last_name'] ?? ''));
    $studentName = '';
    ...
    if (function_exists('appDispatchNotification')) {
        appDispatchNotification(
            ...
            'primary',
            [
                'parent' => "Parent_Chat.php?teacher_id={$teacherId}&student_id={$verifiedStudentId}",
                'default' => "Parent_Chat.php?teacher_id={$teacherId}"
            ],
            [
                'type' => 'chat_message',
