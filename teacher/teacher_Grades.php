...
if ($db) {
    ...
    $hasGradeTerm = (bool)$termColStmt->fetch(PDO::FETCH_ASSOC);
    if (!$hasGradeTerm) {
...
function updateSubmitButtons(students){
  ...
  if(!students||students.length===0){if(submitBtn)submitBtn.style.display='';if(recallBtn)recallBtn.style.display='none';return;}
  const anySubmitted=students.some(function(s){return s.approval_status==='submitted';});
  const allAdminVerified=students.every(function(s){return s.approval_status==='admin_verified';});
  if(anySubmitted&&!allAdminVerified){if(submitBtn)submitBtn.style.display='none';if(recallBtn)recallBtn.style.display='';}
  else if(allAdminVerified){if(submitBtn)submitBtn.style.display='none';if(recallBtn)recallBtn.style.display='';}
  else{if(submitBtn)submitBtn.style.display='';if(recallBtn)recallBtn.style.display='none';}
}
...
document.addEventListener('DOMContentLoaded',()=>{const classEl=document.getElementById('gradeClassSelect');if(classEl&&classEl.value){loadGradesData();}});
