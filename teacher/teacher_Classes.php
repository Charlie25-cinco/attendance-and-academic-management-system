...
function withCsrfUrl(url){if(!csrfToken)return url;return url+(url.includes('?')?'&':'?')+'csrf_token='+encodeURIComponent(csrfToken);}
function appendCsrfToFormData(fd){if(fd&&csrfToken)fd.set('csrf_token',csrfToken);return fd;}
...
function uploadMaterial(){const form=document.getElementById('uploadMaterialForm');const fd=new FormData(form);const title=(fd.get('title')||'').toString().trim();const classId=(fd.get('class_id')||'').toString().trim();const file=fd.get('material_file');const uploadBtn=document.getElementById('uploadMaterialBtn');if(!title||!classId||!file||!(file instanceof File)||file.size===0){showNotification('Please complete all upload fields','warning');return;}setBusy(uploadBtn,'Uploading...','Upload',true);fetch('Teacher_Action.php?action=upload_material',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{if(d.success){showNotification(d.message||'Material uploaded successfully','success');if(uploadMaterialModal)uploadMaterialModal.hide();setTimeout(()=>location.reload(),700);}else{showNotification(d.message||'Failed to upload material','danger');}}).catch(()=>showNotification('Error uploading material','danger')).finally(()=>setBusy(uploadBtn,'Uploading...','Upload',false));}
