...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'get_messages') {
        ...
        // Verify teacher teaches parent's child
        $verifyStmt = $db->prepare("
            SELECT 1 FROM parent_students ps
            JOIN enrollments e ON e.student_id = ps.student_id AND e.status = 'enrolled'
...
if ($action === 'send_message') {
    ...
    // Verify recipient teaches parent's child
    $verifyStmt = $db->prepare("
        SELECT 1 FROM parent_students ps
        JOIN enrollments e ON e.student_id = ps.student_id AND e.status = 'enrolled'
        JOIN class_subjects cs ON cs.class_id = e.class_id AND cs.teacher_id = ?
        WHERE ps.parent_id = ? LIMIT 1
    ");
    $verifyStmt->execute([$toUserId, $parentId]);
    if (!$verifyStmt->fetch()) {
    ...
    $insertStmt = $db->prepare("INSERT INTO messages (from_user_id, to_user_id, student_id, message, created_at)
                                VALUES (?, ?, ?, ?, NOW())");
    $insertStmt->execute([$parentId, $toUserId, $studentId > 0 ? $studentId : null, $message]);
